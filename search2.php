...
<body>
    ...
    if (isset($_GET['search'])) {
        ...
        $check = $_GET['search'];
        if (!empty($check)) {
            $sql_search = "select * from product where (product_type like ?) or (product_id like ?) or (product_type like ?) or (product_detail like ?)  or (product_brand like ?) or (product_name like ?) and product_status = 1 limit $count, $perpage";
            $stmt_search = $conn->stmt_init();
            $stmt_search->prepare($sql_search);
            $stmt_search->bind_param('ssssss', $keyword, $keyword, $keyword, $keyword, $keyword, $keyword);
            $stmt_search->execute();
            $result_search = $stmt_search->get_result();

            $stmt_count = $conn->prepare("select * from product where (product_type like ?) or (product_id like ?) or (product_type like ?) or (product_detail like ?)  or (product_brand like ?) or (product_name like ?) and product_status = 1");
            $stmt_count->bind_param('ssssss', $keyword, $keyword, $keyword, $keyword, $keyword, $keyword);
            $stmt_count->execute();
            $result_count = $stmt_count->get_result();           
            $total_query = $result_count->num_rows;
        } else {
        ...
        if (!empty($check)) {
            ...
            $stmt_search->execute();
            $result_search = $stmt_search->get_result();
            $total_query = $result_search->num_rows;
        } else {
            $sql_limit = "select * from product where product_status = 1  limit $count, $perpage";
            $result_search = $conn->query($sql_limit);
        }
    ...
    <form action="" method="get">
        <div class="container-fluid container-lg" id="box_result" style="padding-left: 2rem; padding-right:2rem;">
            ...
            if (!empty($check)) {
                if (!empty($check2)) {
            ?>
                    <h6 style="font-weight:500; text-align:center; margin-top:1rem;"><i class="bi bi-search">&nbsp;&nbsp;</i>ค้นหา&nbsp;&nbsp;<span style="color:darkred ;"><?php echo $check; ?></span>&nbsp;&nbsp;ยี่ห้อ&nbsp;&nbsp;<span style="color:darkred ;"><?php echo $check2; ?></span></h6>
                <?php
                } else {
                ?>
                    <h6 style="font-weight:500; text-align:center; margin-top:1rem;"><i class="bi bi-search">&nbsp;&nbsp;</i>ค้นหา&nbsp;&nbsp;'<span style="color:darkred ;"><?php echo $check; ?></span>'</h6>
                <?php
                }
                ?>
                    ...
                    ?>
                            <div class="col col-xl-3 col-lg-4 col-md-6 col-sm-6" id="box_product">
                                <div class="card" id="card_product" style="padding-top: 1rem; margin-bottom:1rem;">
                                    ...
                                    ?>
                                    <a href="product.php?id=<?php echo $data['product_id']; ?>"><img style="height:12rem; width:12rem; display:block; margin-left:auto; margin-right:auto; " src="upload/<?php echo $data_picture['picture_name']; ?>" class="card-img" alt="" srcset=""></a>
                                    <div class="card-body">
                ...
                <nav aria-label="">
                    <ul class="pagination">
                        ...
                        if(!isset($total_query)){
                            $sql_total = $conn->query("select * from product where product_status = 1");
                            $total_query = $sql_total->num_rows;
                        }
