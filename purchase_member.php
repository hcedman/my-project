<?php
session_start();
if (!isset($_SESSION['user_id']) && $_GET['member_id'] !== $_SESSION['user_id']) {
...
<body>
    ...
    if ($count_order <= 0) {
        echo "<script>alertInto('info','ไม่มีรายการคำสั่งซื้อ','manage_member.php')</script>";
    }
    ?>
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        if (isset($_GET['status'])) {
            ...
            if ($status != "all") {
                ...
                $sql_order = $conn->query("select * from orders where order_status = '$status'");
